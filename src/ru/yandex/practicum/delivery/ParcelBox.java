...
public class ParcelBox<T extends Parcel> {
    ArrayList<T> getAllParcels = new ArrayList<>();
    ...
    public void getAllParcels() {
        if (getAllParcels.isEmpty()) {
            System.out.println("В этой коробке нет посылок!");
            return;
        }

        for (int i = 0; i < getAllParcels.size(); i++) {
            System.out.println(String.format("Посылка %d. %s", i + 1, getAllParcels.get(i).getDescription()));
        }
    }
}
